...
export default function usePrograms() {
  ...
  const handleReorderTrainings = async (programId: string | null, ids: string[]) => {
    ...
    if (target) {
      target.trainings = ids.map((id) => target.trainings.find((t: any) => t.id === id)!);
      setPrograms(updated);
    }
  };
    ...
    const updatedPrograms = programs.map((p) => {
      if (p.id === programId) {
        ...
        const newTrainings = p.trainings.map((t: Training, i: number) => {
          if (i > index) return { ...t, position: t.position + 1 };
          return t;
        });
        newTrainings.splice(index + 1, 0, duplicated);
        return { ...p, trainings: newTrainings };
      }
      return p;
    });
  ...
  const moveTrainingToAnotherProgram = async (
    ...
    const updatedPrograms = programs.map((program) => {
      ...
      if (program.id === targetProgramId) {
        ...
        return { ...program, trainings: [...filtered.slice(0, newPosition), training, ...filtered.slice(newPosition)] };
      }
      return program;
    });
        ...
        program.id === programId
          ? {
              ...
              trainings: orderedIds
                .map((id) => program.trainings.find((t: typeof program.trainings[number]) => t.id === id))
  ...
  const updateAllProgramPositionsInSupabase = async (programList: Program[]) => {
    await Promise.all(programList.map((p, idx) => supabase.from("programs").update({ position: idx }).eq("id", p.id)));
  };
