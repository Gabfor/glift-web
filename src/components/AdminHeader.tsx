...
export default function AdminHeader() {
  ...
  useEffect(() => {
    const { data } = supabase.auth.onAuthStateChange((_event, session) => {
      const url = (session?.user?.user_metadata?.avatar_url as string) ?? null;
      setAvatarUrl(url);
    });
  ...
  const displayInitial = user?.user_metadata?.name?.trim()?.charAt(0)?.toUpperCase() || "?";
  const displayName = user?.user_metadata?.name?.trim() || "Profil";

  return (
    <header className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${isSticky ? "bg-white shadow-[0_5px_21px_0_rgba(93,100,148,0.15)]" : "bg-[#FBFCFE]"}`}>
      <div className="max-w-[1152px] mx-auto px-4 md:px-0 py-4 flex items-center justify-between">
        <div className="w-[147px] flex items-center">
          <Link href="/admin" className="flex items-center">
            <Image src="/logo_beta.svg" alt="Logo Glift Admin" width={147} height={35} priority />
          </Link>
        </div>

        <nav className="flex-1 flex justify-center gap-6 text-[16px] text-[#5D6494] font-semibold">
          <Link href="/admin/program" className={pathname === "/admin/program" || pathname?.startsWith("/admin/program?") ? "text-[#7069FA]" : "hover:text-[#3A416F]"}>Programmes</Link>
          <Link href="/admin/program-store" className={pathname?.startsWith("/admin/program-store") ? "text-[#7069FA]" : "hover:text-[#3A416F]"}>Store</Link>
          <Link href="/admin/slider" className={pathname?.startsWith("/admin/slider") ? "text-[#7069FA]" : "hover:text-[#3A416F]"}>Slider</Link>
          <Link href="/admin/offer-shop" className={pathname?.startsWith("/admin/offer-shop") ? "text-[#7069FA]" : "hover:text-[#3A416F]"}>Shop</Link>
          <Link href="/admin/content-blog" className={pathname?.startsWith("/admin/content-blog") ? "text-[#7069FA]" : "hover:text-[#3A416F]"}>Blog</Link>
          <Link href="/admin/content-help" className={pathname?.startsWith("/admin/content-help") ? "text-[#7069FA]" : "hover:text-[#3A416F]"}>Aide</Link>
          <Link href="/admin/users" className={pathname?.startsWith("/admin/users") ? "text-[#7069FA]" : "hover:text-[#3A416F]"}>Utilisateurs</Link>
        </nav>
        ...
        <div className="relative ml-[18px]" ref={dropdownRef}>
          {user && (
            <button onClick={() => setDropdownOpen(!dropdownOpen)} className="group flex items-center gap-2 text-[#5D6494] hover:text-[#3A416F] text-[16px] font-semibold">
              {avatarUrl ? (
                <span className="w-[44px] h-[44px] rounded-full overflow-hidden flex items-center justify-center">
                  <Image src={avatarUrl} alt="Avatar" width={44} height={44} className="rounded-full object-cover" />
                </span>
              ) : (
