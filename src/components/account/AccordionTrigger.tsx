...
export default function AccordionTrigger({ children }: { children: ReactNode }) {
  return (
    <BaseTrigger
      className="h-[60px] font-semibold text-[#5D6494] hover:text-[#3A416F] text-[16px] pl-4 pr-6 py-3 hover:no-underline flex items-center justify-between group appearance-none before:hidden after:hidden w-full data-[state=open]:rounded-t-[8px] rounded-[8px]"
    >
      <span>{children}</span>
