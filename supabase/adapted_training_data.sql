...
cross join lateral (
  select
    greatest(
      ...
      round(
        et.base_reps
        + sin(((sd.session_index + gs.set_number)::double precision) / 2.0) * et.rep_spread
        + case
            when sd.session_index % 6 = 0 then -2
            when sd.session_index % 6 = 1 then 1
            when sd.session_index % 6 = 3 then 2
            when sd.session_index % 6 = 4 then -1
            else 0
          end
      )
    )::int as rep_count,
    case
      when et.base_weight = 0 then 0::numeric
      else greatest(
        0::numeric,
        round(
          (
